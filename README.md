...
The `data` directory contains protein structures, auxiliary descriptors, dataset lists, and prediction results used in this project.

- `364/`: Contains the PDB structure files of the benchmark proteins.
- `features_additional/`: Contains auxiliary descriptors used for model construction.
- `list-small.txt`, `list-medium.txt`, `list-large.txt`, and `list-superset.txt`: Protein ID lists for different benchmark datasets.
- `linear_approximation_CAL.csv`: Contains the predicted B-factors generated by the CAL least-square approximation model and the comparison results with other methods.
